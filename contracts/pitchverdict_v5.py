...
SCORING GUIDANCE: Use multiples of 5 for all numeric scores (e.g. 60, 65, 70, 75). Anchor your scores to these reference points: 90-100 exceptional and rare, 75-89 strong, 60-74 solid with real gaps, 40-59 weak, below 40 failing. This keeps scoring consistent across evaluations.
...
Return ONLY the JSON. No commentary, no markdown fences."""
        ...
        principle = (
            "Validators reach consensus when they agree on the DIRECTION and SUBSTANCE of the evaluation, not on exact numbers. "
            "Consensus IS reached when all of the following hold: overall_verdict matches or is within one adjacent tier; "
            "overall_investability values fall within 25 points of each other; and the qualitative content (lens_verdicts, key_strengths, "
            "key_concerns, one_thing_to_fix, investor_question, disagreement_insight) addresses substantively similar themes even if worded differently. "
            "Dimension scores (problem_validity, solution_credibility, market_realism, business_model_strength, traction_signal, team_conviction, "
            "competitive_positioning, moat_potential) are advisory signals and may vary by up to 35 points across validators without blocking consensus. "
            "Do not withhold consensus over numeric precision. Prioritize agreement on the overall investment story and verdict direction over exact score matching. "
            "Each lens_verdict must address what that lens looks for. key_strengths and key_concerns must reference specific evidence from the pitch. "
            "disagreement_insight must name specific lenses and explain the substantive reason for any split when confidence is Contested or Low."
        )
        ...
        seed = submitter_str + ":" + str(counter_now) + ":pitchverdict_v5"
        ...
        global_existing_raw = ""
        if "all" in self.global_index:
            global_existing_raw = self.global_index["all"]

        if global_existing_raw:
            global_ids_list = json.loads(global_existing_raw)
            global_ids_list.append(result_id)
        else:
            global_ids_list = [result_id]

        self.global_index["all"] = json.dumps(global_ids_list)
    ...
    def get_all_results(self) -> list:
        if "all" not in self.global_index:
            return []
        ids_raw = self.global_index["all"]
        ids = json.loads(ids_raw)
        results = []
        i = len(ids) - 1
        while i >= 0:
